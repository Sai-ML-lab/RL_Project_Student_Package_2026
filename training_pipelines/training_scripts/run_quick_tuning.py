...
MODEL_ROOT = PROJECT_ROOT / "training_pipelines" / "models"
...
def _run(kind: str, config: dict, timesteps: int, seed: int, results: list[dict]) -> None:
    ...
    if best_cost is None:
        raise RuntimeError(f"No usable evaluation log found in {run_dir}")
    ...
    print(f"Best quick-eval cost: {best_cost:,.2f}", flush=True)
...
def main() -> int:
    ...
    ranked = sorted(results, key=lambda r: r["best_quick_eval_cost"])
    print("\n=== QUICK-TUNING RANKING ===")
    for rank, row in enumerate(ranked, 1):
        print(
            f"{rank:>2}. {row['kind'].upper():4} "
            f"cost={row['best_quick_eval_cost']:,.2f} "
            f"lr={row['learning_rate']} gamma={row['gamma']} "
            f"target={row['target_update_interval']} "
            f"eps={row['exploration_final_eps']}"
        )
    return 0
