...
def _evaluate_quick(model_path: Path, seed_count: int) -> dict[str, float]:
    ...
    mean_cost = float(match.group(1).replace(",", ""))

    service_match = re.search(r"mean_service_level=([0-9.]+)", output)
    mean_service = float(service_match.group(1)) if service_match else float("nan")
    return {"quick_mean_cost": mean_cost, "quick_mean_service_level": mean_service}
...
def main() -> int:
    ...
    rows = []
    for cfg in selected:
        rows.append(_run(cfg, args.steps, args.seed, args.workers, args.eval_seeds))
    ...
    for rank, row in enumerate(rows, 1):
        ...
        print(
            ...
            f"lambda={cfg['gae_lambda']} ent={cfg['entropy_coef']} "
            f"rollout={cfg['rollout_steps']}"
        )
