...
def main() -> int:
    ...
    run_name = args.run_name or (
        f"ddqn_rep_c_{args.timesteps // 1000}k_lr{args.learning_rate:.0e}_g{args.gamma}_seed{args.seed}"
    )
    save_dir = MODELS_DIR / "double_dqn_rep_c_experiments" / run_name
    save_dir.mkdir(parents=True, exist_ok=True)

    train_env = build_rep_c_vec_env(n_envs=1, seed=args.seed, flatten_action=True, shaping=True, aggregate_anneal_steps=args.timesteps)
    eval_env = build_rep_c_vec_env(n_envs=1, seed=900, flatten_action=True, shaping=False)
    callback = EvalCallback(eval_env, best_model_save_path=str(save_dir), log_path=str(save_dir / "eval_logs"), eval_freq=10_000, n_eval_episodes=10, deterministic=True, verbose=1)
    ...
    model = DoubleDQN(
        ...
        tau=1.0, gamma=args.gamma, train_freq=args.train_frequency,
        gradient_steps=args.gradient_steps, target_update_interval=args.target_update_interval,
        exploration_fraction=args.exploration_fraction, exploration_initial_eps=1.0,
        exploration_final_eps=args.exploration_final_eps, max_grad_norm=10.0,
        policy_kwargs={"net_arch": [256, 256]}, seed=args.seed, verbose=1, device="cpu",
    )
    print(
        "DDQN Rep-C configuration: "
        f"dim={REPRESENTATION_C_DIM}, timesteps={args.timesteps}, lr={args.learning_rate}, "
        f"gamma={args.gamma}, target={args.target_update_interval}, eps={args.exploration_final_eps}", flush=True,
    )
    try:
