"""Portfolio-aware Neural SARSA screening.

This experiment keeps the canonical Neural SARSA algorithm and Representation B
(76-D) but broadens the hyperparameter search around the settings that matter
most for the current weak portfolio member: learning rate, gamma, epsilon end,
buffer size, batch size, and target-network update interval.

Results are intended for screening only. Final candidates must be re-evaluated
with the official 200-episode holdout before any promotion decision.
"""
...
# Make the training_pipelines package layout importable when this file is run
# from the repository root or directly from training_pipelines/.
...
MODELS_DIR = TRAINING_ROOT / "models" / "neural_sarsa_portfolio_screen"
...
CONFIGS = {
    "s1_lr1e-4_g0.99_e0.02_b128_buf100k_t1000": {
        "learning_rate": 1e-4,
        "gamma": 0.99,
        "epsilon_end": 0.02,
        "batch_size": 128,
    ...
    "s2_lr2e-4_g0.99_e0.02_b128_buf100k_t2000": {
        "learning_rate": 2e-4,
        "gamma": 0.99,
        "epsilon_end": 0.02,
        "batch_size": 128,
    ...
    "s3_lr3e-4_g0.99_e0.03_b256_buf100k_t2000": {
        "learning_rate": 3e-4,
        "gamma": 0.99,
        "epsilon_end": 0.03,
        "batch_size": 256,
    ...
    "s4_lr2e-4_g0.98_e0.03_b128_buf100k_t4000": {
        "learning_rate": 2e-4,
        "gamma": 0.98,
        "epsilon_end": 0.03,
        "batch_size": 128,
    ...
    "s5_lr1e-4_g0.98_e0.05_b256_buf100k_t2000": {
        "learning_rate": 1e-4,
        "gamma": 0.98,
        "epsilon_end": 0.05,
        "batch_size": 256,
    ...
    "s6_lr3e-4_g0.98_e0.03_b128_buf50k_t1000": {
        "learning_rate": 3e-4,
        "gamma": 0.98,
        "epsilon_end": 0.03,
        "batch_size": 128,
...
def main() -> int:
    ...
    print(
        f"Running {len(selected)} Neural SARSA configs at {args.total_transitions:,} "
        f"transitions, seed={args.seed}.",
        flush=True,
    )
    ...
    results.sort(key=lambda row: row["best_eval_cost"])
    output = RESULTS_DIR / "screening_results.json"
    output.write_text(json.dumps(results, indent=2), encoding="utf-8")

    print("\n=== Neural SARSA portfolio screening ranking ===")
