...
Final selected candidate: V030 / A2C / Representation C.
The policy is self-contained and performs inference only during evaluation.
"""
...
def run_policy(observation):
    ...
    quantities = [int(x * 10) for x in action]
    if any(q not in range(0, 101, 10) for q in quantities):
        raise ValueError(f"Invalid quantities: {quantities}")
    return quantities
